...
function SavedSourceCard({ source, busy, onUpdate, onDelete }: { source: ResearchWebSource; busy: string; onUpdate: (patch: Partial<ResearchWebSource>) => void; onDelete: () => void }) {
  ...
  useEffect(() => setDraft({
    ...
    review_status: source.review_status,
    summary: source.summary,
  }), [source.authority_level, source.review_status, source.source_kind, source.summary]);
  const isBusy = busy === `update:${source.id}` || busy === `delete:${source.id}`;
  const draftNeedsSummary = draft.review_status !== "candidate";
  const draftCanVerify = draft.review_status !== "verified" || draft.authority_level !== "unverified";
  return <article className={`rounded-2xl border p-4 ${source.review_status === "verified" ? "border-emerald-300/20 bg-emerald-300/5" : source.review_status === "conflict" ? "border-red-300/20 bg-red-300/5" : "border-white/10 bg-slate-950/50"}`}><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-white/10 px-2.5 py-1 text-[11px] font-semibold">{source.review_status}</span><span className="rounded-full bg-white/10 px-2.5 py-1 text-[11px]">{source.authority_level}</span><span className="rounded-full bg-white/10 px-2.5 py-1 text-[11px]">{source.source_kind}</span></div><a href={source.url} target="_blank" rel="noreferrer" className="mt-3 inline-flex max-w-full items-start gap-2 font-semibold text-blue-300 hover:text-blue-200"><span className="line-clamp-2">{source.title || source.domain}</span><ExternalLink className="mt-0.5 shrink-0" size={15} /></a><p className="mt-1 text-xs text-slate-500">{source.domain} · gecontroleerd {new Date(source.checked_at).toLocaleDateString()}</p></div><button type="button" onClick={onDelete} disabled={isBusy} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border border-red-300/20 px-3 py-2 text-xs font-semibold text-red-200 disabled:opacity-40">{busy === `delete:${source.id}` ? <Loader2 className="animate-spin" size={14} /> : <Trash2 size={14} />}Verwijder</button></div><div className="mt-4 grid gap-3 md:grid-cols-3"><select value={draft.source_kind} onChange={(event) => setDraft((current) => ({ ...current, source_kind: event.target.value }))} disabled={isBusy} className="rounded-xl border border-white/15 bg-slate-900 px-3 py-2.5 text-xs">{sourceKinds.map((value) => <option key={value} value={value}>{value}</option>)}</select><select value={draft.authority_level} onChange={(event) => setDraft((current) => ({ ...current, authority_level: event.target.value }))} disabled={isBusy} className="rounded-xl border border-white/15 bg-slate-900 px-3 py-2.5 text-xs">{authorityLevels.map((value) => <option key={value} value={value}>{value}</option>)}</select><select value={draft.review_status} onChange={(event) => setDraft((current) => ({ ...current, review_status: event.target.value }))} disabled={isBusy} className="rounded-xl border border-white/15 bg-slate-900 px-3 py-2.5 text-xs">{reviewStatuses.map((value) => <option key={value} value={value}>{value}</option>)}</select></div><textarea value={draft.summary} onChange={(event) => setDraft((current) => ({ ...current, summary: event.target.value.slice(0, 8000) }))} rows={3} placeholder="Wat ondersteunt of weerspreekt deze bron precies?" className="mt-3 w-full rounded-xl border border-white/15 bg-slate-900 px-4 py-3 text-sm leading-6" /><button type="button" onClick={() => onUpdate(draft)} disabled={isBusy || (draftNeedsSummary && draft.summary.trim().length < 20) || !draftCanVerify} className="mt-3 inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-xs font-semibold text-slate-950 disabled:opacity-40">{busy === `update:${source.id}` ? <Loader2 className="animate-spin" size={14} /> : <Save size={14} />}Review opslaan</button>{draftNeedsSummary && draft.summary.trim().length < 20 && <div className="mt-3 flex gap-2 rounded-xl border border-amber-300/20 bg-amber-300/5 p-3 text-xs leading-5 text-amber-100"><AlertTriangle className="mt-0.5 shrink-0" size={14} />Beschrijf in minimaal 20 tekens wat deze bron precies ondersteunt of weerspreekt.</div>}{!draftCanVerify && <div className="mt-3 flex gap-2 rounded-xl border border-amber-300/20 bg-amber-300/5 p-3 text-xs leading-5 text-amber-100"><AlertTriangle className="mt-0.5 shrink-0" size={14} />Kies official, primary of secondary voordat je deze bron verifieert.</div>}{source.review_status === "candidate" && <div className="mt-3 flex gap-2 rounded-xl border border-amber-300/20 bg-amber-300/5 p-3 text-xs leading-5 text-amber-100"><AlertTriangle className="mt-0.5 shrink-0" size={14} />Kandidaatbronnen tellen niet mee voor de score.</div>}</article>;
}
