...
export default async function handler(req: RequestLike, res: ResponseLike) {
  ...
  if (!xrplResponse.ok || data.error || data.result?.error) {
    return res.status(502).json({
      ok: false,
      error: "XRPL proof stamp lookup failed.",
      details: data,
    });
  }
  ...
  const hasProofMemo =
    memoData.some((memo) => memo.includes("OTT_PROOF")) ||
    memoText.some((memo) => memo.type.includes("OTT_PROOF_STAMP"));

  const proofStampVerified = Boolean(
    validated &&
      success &&
      isPayment &&
      sourceTagMatches &&
